    ...
    'player': { // enemy uses the same model only a different sprite
		...
		animators: { // should be called with the object as this context (model.animators.death.call(model,...))
			'death': function(special){
				...
				} else if ((Date.now() || +new Date) - this.lastAnimator > 100) { // next frame (at least) every 100ms
			...
			'walk': function(special){
				//console.log(special+": "+((Date.now() || +new Date) - this.lastAnimator > 50))
				if (special == 'init') {
					...
					this.currentSprite = 0;
					this.lastAnimator = Date.now() || +new Date;
				} else if ((Date.now() || +new Date) - this.lastAnimator > 50) {
					if (special == 'pause') {}
					else {
